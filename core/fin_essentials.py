...
def cal_fin_data(data, flow_fin_list=(), cross_fin_list=(), discard=True):
    ...
    for col in flow_fin_list:
        ...
        if need_col([col + "_ttm绝对同比"]):
            data[col + "_ttm绝对同比"] = data[col + "_ttm"] - last_4q_df[col + "_ttm"]  # 计算ttm绝对同比

    # ==========处理截面数据
    last_q_df = get_index_data(data, last_q_index, cross_fin_list)  # 获取上个季度的数据
    last_4q_df = get_index_data(data, last_4q_index, cross_fin_list)  # 获取去年同期的数据
...
# 计算财务预处理数据
def merge_with_finance_data(stock_code, stock_df, fin_cols):
    """
    将财务数据合并到日线数据上
    :param stock_code: 股票代码
    :param stock_df: 日线数据
    :param fin_cols: 财务数据列列表
    """
    # 判断路径是否存在
    fin_data_path = data_center_path / "stock-fin-data"
    stock_fin_folder = fin_data_path / stock_code
